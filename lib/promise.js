...
class promise {
	...
	_update() {
		...
		this._queue.forEach(item => {
			const matchedHandlers = this._handlers.filter(handler => this._match_handler(handler, item.event));

			if(error_events.includes(item.event) && matchedHandlers.length === 0) {
				console.error("UnhandledPromiseRejectionWarning: " + item.args[0]);
			}

			matchedHandlers.forEach(matchedHandler => {
				if(!this._handlers.includes(matchedHandler)) {
					return;
				}

				let child;

				if(matchedHandler.event === "then") {
					child = matchedHandler.callback(item.event, ...item.args);
				}
				else {
					child = matchedHandler.callback(...item.args);
				}

				if(child instanceof promise) {
					this._parentify(child, this._handlers.indexOf(matchedHandler) + 1);
				}
				else if(child !== undefined && matchedHandler.options.passthrough) {
					this._parentify(new promise().event(item.event, child), this._handlers.indexOf(matchedHandler) + 1);
				}
			});
		});
	...
	/**
	 * With this function you can check if the specified handler matches the specified event.
	 * @summary Matches the specified handler to the specified event.
	 * @access internal
	 * @param {Object} handler The handler to match to the event.
	 * @param {String} event The evenet to match the handler with.
	 * @returns {Boolean} If the handler matches the event.
	 * @function _match_handler
	 */
	...
	/**
	 * Using this function you can parentify the promise to the specified child and copy the 
	 * specified handlers.
	 * @summary Parentifies the promise to the specified child.
	 * @access internal
	 * @param {Promise} child The promise to set as child.
	 * @param {Number} handlers_index The index from where to start copying handlers to the child.
	 * @function _parentify
	 */

...
class promise {
	constructor(func) {
		assert.strictEqual(typeof func, "function", "The argument func must be a function");
	...
	_update() {
		...
		this._queue.forEach(item => {
			const matches = this._handlers.filter(handler => this._match_handler(handler, item.event));

			matches.forEach(match => {
				if(!this._handlers.includes(match)) {
					return;
				}
				...
				if(match.event === "then") {
					child = match.callback(item.event, ...item.args);
				}
				else {
					child = match.callback(...item.args);
				}

				if(child instanceof promise) {
					this._parentify(child, this._handlers.indexOf(match) + 1);
				}
			});
		});
